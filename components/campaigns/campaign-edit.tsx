...
import { fetchCampaign, fetchCampaignVariants, fetchCampaignContacts, updateCampaign } from "@/lib/services/campaigns"
import type { CampaignFormValues } from "@/lib/schemas/campaign"
import { getCampaignDisplayStatus } from "@/lib/constants/status"
...
export default function CampaignEdit() {
  ...
  if (campaignError || variantsError || contactsError) {
    return <p>Erro ao carregar a campanha. Tente novamente.</p>
  }

  const campaign = campaignResponse?.data
  const variants = variantsResponse?.data
  const audienceContacts = contactsResponse?.data ?? []

  if (isCampaignLoading || isVariantsLoading || isContactsLoading || !campaign) {
  ...
  const initialData: CampaignFormValues = {
    ...
    status: (campaign.derived_status ?? campaign.status) as CampaignFormValues["status"],
    scheduled_at: campaign.scheduled_at ?? null,
    timezone: campaign.timezone ?? "America/Sao_Paulo",
    media_type: campaign.media_type ?? "none",
    media_url: campaign.media_url ?? "",
    message_body: campaign.message_body ?? "",
    link_preview: Boolean(campaign.link_preview),
    ...
    typing_delay_seconds: campaign.typing_delay_seconds ?? 0,
    ...
    audience_source: audienceContacts.length ? "file" : "all",
    audience_contact_ids: audienceContacts.map((contact) => contact.id),
  ...
  return (
    <div className="space-y-4">
      <CampaignForm
        ...
        initialImportedContacts={audienceContacts}
        allowPublishFromDraft={displayStatus === "draft"}
